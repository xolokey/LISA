...
const Sidebar: React.FC = () => {
  ...
  const linkClasses = "flex items-center px-3 py-2.5 font-medium text-text-secondary dark:text-dark-text-secondary hover:bg-gray-100 dark:hover:bg-dark-surface hover:text-text-primary dark:hover:text-dark-text-primary rounded-lg transition-all duration-200";
  const activeLinkClasses = "text-white bg-primary dark:bg-dark-primary shadow-md";

  return (
    <aside className="w-72 bg-surface dark:bg-dark-surface border-r border-border-color dark:border-dark-border-color p-4 flex flex-col shrink-0">
      <div className="flex items-center mb-6 px-2">
        <div className="w-9 h-9 bg-primary rounded-lg mr-3 flex items-center justify-center font-bold text-white text-xl">
            L
        </div>
        <span className="text-xl font-bold text-text-primary dark:text-dark-text-primary">Lisa</span>
      </div>
        ...
        <div className="pt-2">
          {user ? (
            <div className="flex items-center gap-3">
              ...
              <button onClick={signOut} title="Sign Out" className="text-secondary dark:text-dark-secondary hover:text-primary dark:hover:text-dark-primary transition-colors p-1">
                {ICONS.signOut}
              </button>
            </div>
          ) : (
